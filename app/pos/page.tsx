  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (currentStoreId) {
          const { data: productData } = await supabase.from("products").select("*").eq("store_id", currentStoreId);
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans pb-24 md:pb-0">
      <header className="bg-white shadow-sm px-4 md:px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          {store?.logo_url ? (
            <div className="w-12 h-12 relative rounded-md overflow-hidden bg-white shadow-sm border border-gray-100">
              <Image src={store.logo_url} alt="Logo" fill className="object-contain p-1" unoptimized />
            </div>
          ) : (
        ...
        {/* ใช้ router.push แทน window.location.href ตามมาตรฐาน Next.js */}
        <button onClick={() => router.push('/')} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-lg text-sm transition-colors border border-gray-200">
          กลับหน้าหลัก
        </button>
      </header>
      ...
      <main className="flex-1 flex flex-col md:flex-row p-4 md:p-6 gap-6 md:overflow-hidden">
        <div className="flex-1 md:overflow-y-auto md:pr-2">
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
            {products.map((product) => (
              <div key={product.id} onClick={() => addToCart(product)} className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center cursor-pointer active:scale-95 hover:shadow-md hover:border-blue-300 transition-all">
                <div className="w-full aspect-square bg-gray-50 rounded-xl mb-3 flex items-center justify-center relative overflow-hidden border border-gray-100 p-2">
                  {product.image_url ? (
                    ...
                    <span className="text-gray-400 text-sm font-medium">ไม่มีรูป</span>
                  )}
                </div>
                <h3 className="font-semibold text-center text-sm md:text-base line-clamp-2 min-h-[2.5rem] w-full text-gray-800">{product.name}</h3>
                <p className="text-blue-600 font-extrabold mt-1 text-base md:text-lg">฿{(product.sell_price || 0).toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`${isMobileCartOpen ? "fixed inset-0 z-50 flex" : "hidden"} md:flex w-full md:w-[350px] lg:w-[400px] flex-col bg-gray-900/50 md:bg-transparent`}>
          <div className="bg-white w-full h-full md:h-[calc(100vh-120px)] md:rounded-2xl shadow-2xl md:shadow-sm border-0 md:border border-gray-100 flex flex-col mt-auto md:mt-0 rounded-t-3xl md:sticky md:top-6 overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-gray-50 md:bg-white">
              <h2 className="text-lg font-bold flex items-center gap-2 text-gray-800">🛒 ตะกร้า <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-sm">{totalItems}</span></h2>
              <button className="md:hidden w-8 h-8 flex items-center justify-center bg-gray-200 rounded-full font-bold text-gray-600" onClick={() => setIsMobileCartOpen(false)}>X</button>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 bg-gray-50/50">
              {cart.length === 0 ? <div className="h-full flex items-center justify-center text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-xl bg-white">ยังไม่ได้เลือกสินค้า</div> : 
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                      <div className="flex-1">
                        <p className="font-semibold text-sm line-clamp-1 text-gray-800">{item.name}</p>
            ...
            <div className="border-t p-4 bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-10">
              <div className="flex justify-between font-bold text-lg mb-4 text-gray-800"><span>ยอดรวม</span><span className="text-blue-600 text-xl">฿{totalPrice.toLocaleString()}</span></div>
              <button 
                onClick={handleCheckout}
                className={`w-full py-4 rounded-xl font-bold text-lg transition-all shadow-lg ${totalItems > 0 && !isSubmitting ? "bg-blue-600 text-white hover:bg-blue-700 shadow-blue-200" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
      ...
      {!isMobileCartOpen && (
        <div className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 px-6 flex justify-between items-center z-40 pb-6 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
          ...
          <button onClick={() => setIsMobileCartOpen(true)} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-bold shadow-lg shadow-blue-200 active:scale-95">ดูตะกร้า</button>
        </div>
      )}
    </div>
  );
